...
class Directory:
    def __init__(self, case_name: str = "default_case"):
        ...
        for sub in [
                "mesh", "input", "inp", "msg", "dat", "odb", "report", "data", "fig", "log", "trace", "sta", "temp"
                ]:
...
class RVE:
    def __init__(self, variables=None, directory=None, case_number: int = 0):
        ...
        if variables: self.var.update(variables)

        # saving independent variables
        save_object(self.var, os.path.join(directory.case_folder, "input", f"{case_number}_UC"), method="json")
    ...
    def zpr_core_geometry(self):
        ...
        for key in self.derived_var["coordinates"]:
            self.derived_var["coordinates"][key] = np.array(self.derived_var["coordinates"][key], dtype=float)
        ...
        self.derived_var["solid_surface_connectivity"] = [
                [[1, "1_5"], [1, "5_6"], [-1, "2_6"], [-1, "1_2"]], [[1, "2_6"], [1, "6_7"], [-1, "3_7"], [-1, "2_3"]],
    ...
    def generate_mesh(self):
        ...
        def create_surface(connectivity: list, lines: dict) -> int:
            ...
            Args:
                connectivity (list of tuple): A list where each tuple contains orientation (int:+1/-1) and identifier (str) for the curve.
        ...
        def create_solid(surfaces: list, extrude_height: float, num_layers: int) -> list[int]:
            ...
            solid = np.asarray(
                    CAD.extrude(
                            surfaces, 0.0, 0.0, extrude_height, numElements=[num_layers], recombine=True, )
                    )
        ...
        def create_cad():
            """
            Constructs the CAD geometry and mesh for representative volume element (RVE) based on the RVE's derived variables and mesh parameters.
            """
            # create nodes
            NODES: dict[str, int] = {}
            ...
            for line_set in self.derived_var["line_connectivity"]:
                line_lengths: list = [np.linalg.norm(
                        self.derived_var["coordinates"][str(a)] - self.derived_var["coordinates"][str(b)]
                        ) for a, b in line_set]
                num_nodes: int = (int(np.ceil(max(line_lengths) / self.var["element_size"])) + 1)
                for a, b in line_set:
                    line_name = f"{a}_{b}"
            ...
            CORE_SURFACES: list[int] = [create_surface(surface, lines=LINES) for surface in
                                        self.derived_var["solid_surface_connectivity"]]
            ...
            if self.var["facesheet_thickness"] > 0.0:
                ...
                FACESHEET_SURFACES = [create_surface(surface, LINES) for surface in
                                      self.derived_var["facesheet_surfaces_connectivity"]]
                # extrude facesheet surfaces to create solid
                num_layers: int = int(
                        np.ceil(
                                self.var["facesheet_thickness"] / self.var["element_size"]
                                )
                        )
                FACESHEET_SOLIDS: list[int] = create_solid(
                        list(
                                (2, surface) for surface in FACESHEET_SURFACES + CORE_SURFACES
                                ), self.var["facesheet_thickness"], num_layers, )
            else:
                # if facesheet does not exist, just translate core surfaces
                CAD.translate(
                        [(2, s) for s in CORE_SURFACES], dx=0.0, dy=0.0, dz=self.var["facesheet_thickness"], )
            ...
            for nodeLabel in self.derived_var["core_end_surface_box"]:
                ...
                CORE_EXTRUDED_FACES.extend(
                        MODEL.getEntitiesInBoundingBox(
                                min_x - eps, min_y - eps, -self.var["core_thickness"] / 2.0 - eps, max_x + eps,
                                max_y + eps, -self.var["core_thickness"] / 2.0 + eps, 2, )
                        )

            # extrude core surfaces to create solid
            num_layers: int = int(
                    np.ceil(
                            (self.var["core_thickness"] / 2.0) / self.var["element_size"]
                            )
                    )
        ...
        def tesselate(mirror_steps: list | None, pattern_steps: list | None) -> None:
            ...
            def reorder_nodes(e, m, tx: int, ty: int, tz: int) -> list[np.ndarray]:
                ...
                if num_nodes_per_element == 8:
                    ...
                    raise (f"Error: Unsupported number of nodes per element: {num_nodes_per_element}")
                ...
                ordered_nodes: list[np.ndarray] = [m[e][2][2][0][list(
                        element_start_index + ordered_index for ordered_index in node_index
            ...
            def transform(
                    m: dict, offset_entity: int, offset_node: int, offset_element: int, mirror_step: list | None,
                    pattern_step: list | None, ) -> None:
                """
                ...
                for e in sorted(m):
                    MODEL.addDiscreteEntity(
                            ...
                            [(abs(b[1]) + offset_entity) * np.copysign(1, b[1]) for b in m[e][0]], )
                    coords = m[e][1][1]
            ...
            def mirror(mirror_steps: list) -> None:
                ...
                for count, mirror_step in enumerate(mirror_steps, start=1):
                    transform(
                            m, int(count * 10 ** 4), int(count * 10 ** 7), int(count * 10 ** 6), mirror_step, None, )
                MESH.renumberNodes()
            ...
            def pattern(pattern_steps: list) -> None:
                ...
                for axis_index, axis_units in enumerate(pattern_steps):
                    ...
                    for count in range(1, axis_units):
                        ...
                        transform(
                                m, int(count * 10 ** (5 + axis_index)), int(count * 10 ** (7 + axis_index)),
                                int(count * 10 ** (6 + axis_index)), None, [
        ...
        def create_physical_groups():
            ...
            FACESHEET_TOP = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymin - eps), (zmax - self.var["facesheet_thickness"] - eps), (xmax + eps),
                    (ymax + eps), (zmax + eps), 3
                    )
            FACESHEET_BOTTOM = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymin - eps), (zmin - eps), (xmax + eps), (ymax + eps),
                    (zmin + self.var["facesheet_thickness"] + eps), 3
                    )
            CORE = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymin - eps), (-self.var["core_thickness"] / 2.0 - eps), (xmax + eps), (ymax + eps),
                    (self.var["core_thickness"] / 2.0 + eps), 3
                    )
            TOP_FACE = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymin - eps), (zmax - eps), (xmax + eps), (ymax + eps), (zmax + eps), 2
                    )
            BOTTOM_FACE = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymin - eps), (zmin - eps), (xmax + eps), (ymax + eps), (zmin + eps), 2
                    )
            LEFT_FACE = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymin - eps), (zmin - eps), (xmin + eps), (ymax + eps), (zmax + eps), 2
                    )
            RIGHT_FACE = MODEL.getEntitiesInBoundingBox(
                    (xmax - eps), (ymin - eps), (zmin - eps), (xmax + eps), (ymax + eps), (zmax + eps), 2
                    )
            FRONT_FACE = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymin - eps), (zmin - eps), (xmax + eps), (ymin + eps), (zmax + eps), 2
                    )
            REAR_FACE = MODEL.getEntitiesInBoundingBox(
                    (xmin - eps), (ymax - eps), (zmin - eps), (xmax + eps), (ymax + eps), (zmax + eps), 2
                    )

            MODEL.addPhysicalGroup(
                    3, list(item[1] for item in FACESHEET_TOP + FACESHEET_BOTTOM), name='FACESHEET'
                    )  # FACESHEET: MAT-2
        ...
        def create_mesh() -> None:
            """
            Generate the mesh along with the mirror and pattern steps required for the RVE model, and saves it in ABAQUS format.
            """

            # mesh attributes
            meshparams = {
                    "C3D20"            : {
                            "ELEMENT_SHAPE": "BLOCK", "ELEMENT_ORDER": 2, "REDUCED_INTEGRATION": False,
                            }, "C3D20R": {
                            "ELEMENT_SHAPE": "BLOCK", "ELEMENT_ORDER": 2, "REDUCED_INTEGRATION": True,
                            }, "C3D8"  : {
                            "ELEMENT_SHAPE": "BLOCK", "ELEMENT_ORDER": 1, "REDUCED_INTEGRATION": False,
                            }, "C3D8R" : {
                            "ELEMENT_SHAPE": "BLOCK", "ELEMENT_ORDER": 1, "REDUCED_INTEGRATION": True,
                            }
                    }
            ...
            gmsh.write(os.path.join(self.directory.case_folder, "mesh", f"{self.case_number}_RVE_geometry.inp"))
    ...
    def generate_inp(self):
        ...
        def read_mesh_data():
            ...
            with open(
                    os.path.join(self.directory.case_folder, "mesh", f"{self.case_number}_RVE_geometry.inp"), "r"
                    ) as file:
                lines = file.read().splitlines()

            # reach contents of the mesh file
            num_nodes_per_element = int(self.var["element_type"].strip('C3D').strip('R')) + 1
            temp_list = []
            section = None
            current_set = None
            for line in lines:
                if line.startswith('*NODE'):
                    ...
                    if section == "nodes":
                        ...
                        if len(temp_list) == num_nodes_per_element:  # number of nodes in each element
                            RVE_mesh["elements"].append(temp_list)  # element number and node numbers
                            temp_list = []
                    elif section == "elsets":
            ...
            for key in RVE_mesh["elsets"]:
                RVE_mesh["elsets"][key] = np.array(RVE_mesh["elsets"][key], dtype=np.int32)
            for key in RVE_mesh["nsets"]:
                RVE_mesh["nsets"][key] = np.array(RVE_mesh["nsets"][key], dtype=np.int32)

        def center_and_renumber_mesh():
            """Renumbers elements, and centers the model."""

            original_element_number = RVE_mesh["elements"][:, 0].copy()
            RVE_mesh["elements"][:, 0] = np.arange(1, RVE_mesh["elements"].shape[0] + 1, dtype=np.int32)
            for key in RVE_mesh["elsets"]:
                elset_indices = np.searchsorted(original_element_number, RVE_mesh["elsets"][key])
        ...
        def add_pin_and_control_nodes():
            ...
            node_central_idx = np.argmin(np.sum(np.square(RVE_mesh["nodes"][:, 1:]), axis=1))
        ...
        def find_node_pairs() -> np.ndarray:
            ...
            edge_nodes = np.intersect1d(RVE_mesh["nsets"]["XPOS"], RVE_mesh["nsets"]["YPOS"])
            ynode_pairs_filtered = ynode_pairs[~np.isin(ynode_pairs[:, 0], edge_nodes)]

            return np.vstack((xnode_pairs, ynode_pairs_filtered))

        def generate_constraint_equations(node_pairs):  # TODO add transverse shear loading
        ...
        def write_input_file(equations=None):
            ...
            lines = ["*NODE"]
            lines.extend([line for node in RVE_mesh["nodes"] for line in format_lines(node)])

            lines.append(f"*ELEMENT, TYPE={self.var["element_type"]}")
            lines.extend([line for element in RVE_mesh["elements"] for line in format_lines(element)])
            ...
            bool_reduced_integration = True if self.var["element_type"][-1] == "R" else False
            ...
            if "CORE" in RVE_mesh["elsets"].keys():
                lines.extend(
                        [
                                "*MATERIAL, NAME=MAT-CORE", "*ELASTIC",
            ...
            if "FACESHEET" in RVE_mesh["elsets"].keys():
                lines.extend(
                        [
                                "*MATERIAL, NAME=MAT-FACESHEET", "*ELASTIC",
            ...
            load_cases = [
                    ("E11", "STRAIN", 1), ("E22", "STRAIN", 2), ("E12", "STRAIN", 3), ("K11", "CURVATURE", 1),
                    ("K22", "CURVATURE", 2), ("K12", "CURVATURE", 3),
            ...
            if not output_requested:
                lines.extend(
                        [
                                "*OUTPUT, FIELD", "NODE OUTPUT", "CF, RF, U", "*ELEMENT OUTPUT, DIRECTIONS=YES",
                                "E, LE, S, TRSHR", "*OUTPUT, FIELD, VARIABLE=PRESELECT", "*OUTPUT, HISTORY, FREQUENCY=0"
                                ]
                        )
            ...
            with open(os.path.join(self.directory.case_folder, "inp", f"{self.case_number}_RVE.inp"), "w") as f:
                f.write("\n".join(lines))

        # output container
        RVE_mesh = {"nodes": [], "elements": [], "element_type": self.var["element_type"], "elsets": {}, "nsets": {}}
    ...
    def run_abaqus(self, abaqus_path: str = "C:\\SIMULIA\\Commands\\abaqus.bat", num_core: int = 1):
        ...
        try:
            ...
            with open(self.log_file, 'a') as f:
                f.write(f"ERROR: Cannot run Abaqus for case number {self.case_number}\n")

        # Copy result files
        for ext in ["odb", "msg", "dat", "sta"]:
            src = os.path.join(self.run_folder, f"{self.case_number}_RVE.{ext}")
            dst = os.path.join(self.directory.case_folder, ext, f"{self.case_number}_RVE.{ext}")
            try:
    ...
    def analysis(self):
        ...
        self.generate_inp()
        # Job definition
        self.job_file = os.path.join(self.directory.case_folder, "inp", f"{self.case_number}_RVE.inp")
        self.log_file = os.path.join(self.directory.case_folder, "log", f"{self.case_number}_RVE_Abaqus.log")
        self.run_folder = os.path.join(self.directory.abaqus_folder, f"{self.case_number}")
