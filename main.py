...
def logger(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            print(
                f"\t{func.__name__} for {args[0].directory.case_name} - {args[0].case_number} completed in "
                f"{time.time() - start:.2f}s"
            )
            return result
        except:
            print(
                f"\t{func.__name__} for {args[0].directory.case_name} - {args[0].case_number} failed to complete"
            )
...
def plot_stiffness_bars(directory, jobs):
    ...
    for job in jobs:
        ...
        abd = load_object(
            os.path.join(directory.case_folder, "data", f"{case_number}_stiffness"),
            "json",
        )
        tp = load_object(
            os.path.join(directory.case_folder, "input", f"{case_number}_UC_derived"),
            "json",
        )["lz"]
        ...
        plot_data["In-plane"]["E1"].append(
            (abd["A11"] * (1 - (abd["A12"] ** 2) / (abd["A11"] * abd["A22"]))) / tp
        )
        plot_data["In-plane"]["E2"].append(
            (abd["A22"] * (1 - (abd["A12"] ** 2) / (abd["A11"] * abd["A22"]))) / tp
        )
        ...
        plot_data["Out-of-plane"]["E1"].append(
            (abd["D11"] * (1 - (abd["D12"] ** 2) / (abd["D11"] * abd["D22"])))
            * 12
            / (tp**3)
        )
        plot_data["Out-of-plane"]["E2"].append(
            (abd["D22"] * (1 - (abd["D12"] ** 2) / (abd["D11"] * abd["D22"])))
            * 12
            / (tp**3)
        )
    ...
    fig, axes = plt.subplots(
        len(params), len(planes), figsize=(8, 15), sharex="col", sharey="row"
    )
    ...
    # Save the figure
    fig.savefig(
        os.path.join(directory.case_folder, "fig", "equivalent_stiffness.png"),
        bbox_inches="tight",
        dpi=300,
    )
...
if __name__ == "__main__":
    ...
    example = [
        ...
        {},  # accordion core only x symmetry of quarter cell unit so 2 units along y direction for size consistency
        {
            ...
            "num_tiles": [1, 2],
        },  # positive Poisson's ratio honeycomb core
        {"core_type": "ppr"},  # negative Poisson's ratio honeycomb core
        {"core_type": "npr"},
    ]
